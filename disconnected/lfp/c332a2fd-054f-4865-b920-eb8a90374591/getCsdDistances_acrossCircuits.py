...
if __name__=='__main__':
    ...
    ## Loads signals 
    signals = list(np.zeros(5))
    originalSignals = list(np.zeros(5))
    
    for i in range(5):
        for file in os.listdir(str(i)+'/pkls'):
            signals[i] += signal.sosfilt(sos,pd.read_pickle(str(i)+'/pkls/'+file).values,axis=0)[2000:2050]

        for file in os.listdir('../../../original/lfp/5c60add3-8d47-411c-9659-1dcfcaf28aa4/'+str(i)+'/pkls'):
            originalSignals[i] += signal.sosfilt(sos,pd.read_pickle('../../../original/lfp/5c60add3-8d47-411c-9659-1dcfcaf28aa4/'+str(i)+'/pkls/'+file).values,axis=0)[2000:2050]
    ...
    for i in range(5):
        ...
        totalLFPOriginal = originalSignals[i][:,lfpIdx]

        lfp_data = totalLFPOriginal.T * 1E-3 * pq.V        # [mV] -> [V]
        ...
        csdFullOriginal.append(calculateCsd(std_input))

    ####### Calculate distances

    distanceMat = np.zeros([5])

    for i in range(5):

            distanceMat[i] = getDistance(csdFull[i],csdFullOriginal[i])


    np.save('distances_across_circuits.npy',distanceMat)
