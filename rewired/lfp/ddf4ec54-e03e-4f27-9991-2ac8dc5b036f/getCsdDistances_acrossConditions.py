...
if __name__=='__main__':
    ...
    for i in range(5):
        for file in os.listdir(str(i)+'/pkls'):
            signals[i] += signal.sosfilt(sos,pd.read_pickle(str(i)+'/pkls/'+file).values[0:len(pd.read_pickle(str(i)+'/pkls/'+file).values):10],axis=0)
    ...
    otherCsds = [upscaleCsd(csd_half,2), upscaleCsd(csd_quarter,4), objective_sphere_full, upscaleCsd(objective_sphere_half,2) , upscaleCsd(objective_sphere_quarter,4), objective_disk_full, upscaleCsd(objective_disk_half,2), upscaleCsd(objective_disk_quarter,4)]

    ####### Calculate distances

    distanceMat = np.zeros([5,len(otherCsds)])

    for i in range(5):
        for j in range(len(otherCsds)):

            distanceMat[i,j] = getDistance(csdFull[i],otherCsds[j][i])


    np.save('distances_across.npy',distanceMat)
